/**
 * The power of an integer x is defined as the
 * number of steps needed to transform x into 1 using the following steps:
 * 
 ...
 * it is guaranteed that x will transform into 1 using these steps and
...
var getKth = function (lo, hi, k) {
  ...
  numArr = numArr.sort((a, b) => stepsToTransform(a) - stepsToTransform(b));
  return numArr[k - 1];
};

function stepsToTransform(x) {
  let count = 0;
  while (x != 1) {
    if (x % 2 == 0) {
      x = x / 2;
