...
var floodFill = function(image, sr, sc, color) {
    ...
    recursefill(image, sr, sc, color, startColor);
    return image
};

function recursefill(image, x, y, color, startColor){
    if(x>=image.length || y>=image[0].length || x<0 || y<0) return;
    if(image[x][y] != startColor) return
    if(image[x][y] == color) return image;
    image[x][y] = color;
    recursefill(image, x-1, y, color, startColor);
    recursefill(image, x+1, y, color, startColor);
    recursefill(image, x, y-1, color, startColor);
    recursefill(image, x, y+1, color, startColor);
}